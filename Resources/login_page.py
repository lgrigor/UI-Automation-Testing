...
class Login(Common):
    ...
    USER_ERROR_CONTAINER = (By.XPATH, "//h3[@data-test='error']")
    USER_ERROR_BUTTON = (By.CLASS_NAME, "error-button")
    ...
    WRONG_USERNAME_OR_PASSWORD_ERROR_MESSAGE = "Epic sadface: Username and password do not match any user in this service"
    ...
    def verify_blocked_user_error_message_is_visible(self, timeout):
        error_message = self.wait_until_element_visible(locator=self.USER_ERROR_CONTAINER, timeout=timeout)
        if bool(error_message):
            return error_message.text == self.BLOCKED_USER_ERROR_MESSAGE
        else:
            return False
    ...
    def verify_missing_username_error_message_is_visible(self, timeout):
        error_message = self.wait_until_element_visible(locator=self.USER_ERROR_CONTAINER, timeout=timeout)
        if bool(error_message):
            return error_message.text == self.MISSING_USERNAME_ERROR_MESSAGE
        else:
            return False
    ...
    def verify_missing_password_error_message_is_visible(self, timeout):
        error_message = self.wait_until_element_visible(locator=self.USER_ERROR_CONTAINER, timeout=timeout)
        if bool(error_message):
            return error_message.text == self.MISSING_PASSWORD_ERROR_MESSAGE
        else:
            return False
    ...
    def verify_wrong_username_or_password_error_message_is_visible(self, timeout):
        error_message = self.wait_until_element_visible(locator=self.USER_ERROR_CONTAINER, timeout=timeout)
        if bool(error_message):
            return error_message.text == self.WRONG_USERNAME_OR_PASSWORD_ERROR_MESSAGE
        else:
            return False

    def verify_error_close_button_is_visible(self, timeout):
        error_button = self.wait_until_element_visible(locator=self.USER_ERROR_BUTTON, timeout=timeout)
        return bool(error_button)